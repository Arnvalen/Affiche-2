...
- **Sidebar gauche** (360px, rétractable) : éditeur avec 8 onglets
...
- **Composition de la ligne** : les machines sont organisées par zone (étape), avec possibilité de :
...
| State | Un seul `useState` dans App, mis à jour via `up()` (clone profond + mutation) |
